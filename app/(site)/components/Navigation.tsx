...
export default function Navigation() {
  ...
  return (
    <motion.nav
      ...
      className={`sticky top-0 left-0 right-0 flex justify-center z-50 h-16 md:h-20 transition-all duration-300 ${scrolled ? "bg-[#0D1117]/70 backdrop-blur-[10px]" : "bg-transparent"}`}
    >
        ...
        <ul className="hidden lg:flex space-x-6 xl:space-x-8 text-sm">
          {navLinks.map((link) => (
            <li key={link.path} className="nav-link relative">
              <Link href={link.path}>
                <motion.span
                  className={`font-semibold tracking-wider transition-all duration-300 relative ${pathname === link.path ? "text-brand-primary" : "text-brand-textSecondary hover:text-brand-primary"}`}
                >
                  {link.name}
                  {pathname === link.path && (
                    <motion.div
